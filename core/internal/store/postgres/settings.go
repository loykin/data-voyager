...
func (r *settingsRepo) Get(ctx context.Context, key string) (*settings.Setting, error) {
	var row settingRow
	err := r.db.GetContext(ctx, &row, `SELECT * FROM ai_settings WHERE key = $1`, key)
...
func (r *settingsRepo) Set(ctx context.Context, key, value string, isSecret bool) error {
	...
	_, err = r.db.ExecContext(ctx, q, newID.String(), key, value, isSecret, time.Now().UTC())
	if err != nil {
...
func (r *settingsRepo) Delete(ctx context.Context, key string) error {
	_, err := r.db.ExecContext(ctx, `DELETE FROM ai_settings WHERE key = $1`, key)
	if err != nil {

...
// NewAIConfigRepo returns an aiconfig.Repository backed by PostgreSQL.
...
func (r *aiConfigRepo) GetByID(ctx context.Context, id string) (*aiconfig.AIConfig, error) {
	var row aiConfigRow
	err := r.db.GetContext(ctx, &row, `SELECT * FROM ai_configs WHERE id = $1`, id)
...
func (r *aiConfigRepo) Update(ctx context.Context, cfg *aiconfig.AIConfig) error {
	...
	const q = `
		UPDATE ai_configs
		SET name=$1, provider=$2, api_key=$3, model=$4, base_url=$5, is_active=$6, updated_at=$7
		WHERE id=$8`
	_, err := r.db.ExecContext(ctx, q,
...
func (r *aiConfigRepo) Delete(ctx context.Context, id string) error {
	_, err := r.db.ExecContext(ctx, `DELETE FROM ai_configs WHERE id = $1`, id)
	if err != nil {
...
func (r *aiConfigRepo) SetActive(ctx context.Context, id string) error {
	...
	var count int
	if err := tx.QueryRowContext(ctx, `SELECT COUNT(*) FROM ai_configs WHERE id = $1`, id).Scan(&count); err != nil {
	...
	if _, err := tx.ExecContext(ctx, `UPDATE ai_configs SET is_active = 1 WHERE id = $1`, id); err != nil {
